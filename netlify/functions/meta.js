...
exports.handler = async(event)=>{
  ...
  try{
    ...
    if(endpoint==='campaigns'){
      data=await fetchURL(`https://${API}/v19.0/${ACCOUNT}/campaigns?fields=id,name,status,objective,daily_budget,lifetime_budget&limit=50&access_token=${TOKEN}`);
    }else if(endpoint==='insights'){
      data=await fetchURL(`https://${API}/v19.0/${ACCOUNT}/insights?fields=campaign_id,spend,impressions,reach,clicks,inline_link_clicks,ctr,cpm,cost_per_inline_link_click,cost_per_action_type,frequency&date_preset=last_30d&level=campaign&limit=50&access_token=${TOKEN}`);
    }else{
